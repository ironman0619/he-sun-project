...
results <- data.frame(k_value = numeric(), train_accuracy = numeric(), train_log_loss = numeric(), train_calibration = numeric(),
...
for (k in k_values) {
  ...
  print(paste("k-value:", k, 
              ...
              "Training Log Loss:", train_metrics$log_loss, 
              "Training Calibration:", train_metrics$calibration, 
              "Test Accuracy:", test_metrics$accuracy, 
              "Test Log Loss:", test_metrics$log_loss, 
              "Test Calibration:", test_metrics$calibration))
}
...
# Best parameters based on analysis
best_delta <- 50
best_nu <- 1
best_sigma <- 0.05
...
# Append advanced Elo model metrics with best parameters to overall performance metrics
performance_metrics <- rbind(performance_metrics, elo_538_metrics_best)
...
# Define function to update Elo ratings with k = 25
update_elo_25 <- function(winner_rating, loser_rating, winner_matches, loser_matches) {
  expected_winner <- expected_elo_score(winner_rating, loser_rating)
  k_factor <- 25
  new_winner_rating <- winner_rating + k_factor * (1 - expected_winner)
  new_loser_rating <- loser_rating + k_factor * (0 - (1 - expected_winner))
...
# Function to evaluate k = 25 Elo model on a subset of data
evaluate_elo_25 <- function(train_data, validation_data, test_data, ratings, update_func, prob_func) {
  # Initialize Elo ratings and match counts
...
# Evaluate the k = 25 Elo model on top 50 and top 100 players
elo_25_metrics_top_50 <- evaluate_elo_25(train_set_top_50, validation_set_top_50, test_set_top_50, elo_ratings, update_elo_25, calculate_elo_probs_538)
elo_25_metrics_top_100 <- evaluate_elo_25(train_set_top_100, validation_set_top_100, test_set_top_100, elo_ratings, update_elo_25, calculate_elo_probs_538)

# Compile results
results_elo_25_top_50 <- data.frame(
  dataset = c("ELO 25 Top 50 Training", "ELO 25 Top 50 Validation", "ELO 25 Top 50 Testing"),
  accuracy = c(elo_25_metrics_top_50$train$accuracy, elo_25_metrics_top_50$validation$accuracy, elo_25_metrics_top_50$test$accuracy),
  log_loss = c(elo_25_metrics_top_50$train$log_loss, elo_25_metrics_top_50$validation$log_loss, elo_25_metrics_top_50$test$log_loss),
  calibration = c(elo_25_metrics_top_50$train$calibration, elo_25_metrics_top_50$validation$calibration, elo_25_metrics_top_50$test$calibration)
)

results_elo_25_top_100 <- data.frame(
  dataset = c("ELO 25 Top 100 Training", "ELO 25 Top 100 Validation", "ELO 25 Top 100 Testing"),
  accuracy = c(elo_25_metrics_top_100$train$accuracy, elo_25_metrics_top_100$validation$accuracy, elo_25_metrics_top_100$test$accuracy),
  log_loss = c(elo_25_metrics_top_100$train$log_loss, elo_25_metrics_top_100$validation$log_loss, elo_25_metrics_top_100$test$log_loss),
  calibration = c(elo_25_metrics_top_100$train$calibration, elo_25_metrics_top_100$validation$calibration, elo_25_metrics_top_100$test$calibration)
)

# Append results to overall performance metrics
performance_metrics <- rbind(performance_metrics, results_elo_25_top_50, results_elo_25_top_100)
